import math, time, csv, json, subprocess
...
# --- Formal Gate config ---
DELTA_WCET = 0.20        # WCET safety margin (+20%)
...
# ----------------- Formal Gate (PDC + optional SMT/Z3) -----------------
EPS = 1e-9
...
def build_jobs_from_waypoints(waypoints_rel, start_xy, v_cap, w_cap, delta, release_abs):
    """Return list of jobs {name, release, deadline, wcet} using relative deadlines (sec since t0)."""
    ...
    for i, (wp, prev) in enumerate(zip(waypoints_rel, pts)):
        ...
        jobs.append({
            "name": f"leg{i}",
            "release": float(release_abs),                # all released at t0 for now
...
def minimal_repair_hint(cert):
    ...
    v = min(cert["violations"], key=lambda r: r["margin"])  # worst violation
    delta = -(v["margin"])
    return {
...
V_CAP_FOR_WCET = min(V_MAX, worst_v_cap_from_table(VMAX_TABLE))  # most conservative linear speed
...
# ----------------- Main loop -----------------
try:
    export_event(trace, 0.0, "start", None, note="formal-gate passed; constrained+STL runtime")
    last_pose_log = t0
    while True:
        t = now()
        x, y, th = pose_xyth()
        ...
        if current is not None:
            ...
            mon_inGoal.step(-1e6)  # unreachable until a goal is active
