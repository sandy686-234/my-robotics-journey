...
import math, time, csv, json, subprocess, re
...
# 是否把 PDC 窗口也写成 SMT 断言（通常不需要，交给 Python 侧判定）
...
def segment_intersects_rect(p, q, rect):
    """Axis-aligned rectangle intersection (AABB) as a conservative corridor check."""
...
def export_smt2_multi(cert, jobs, eps_mutex, path):
    ...
    Generate multi.smt2 that:
      - (optionally) lists PDC windows (named, for unsat-core); by default not asserted
    ...
    # PDC windows for traceability (do NOT assert them by default)
    for i, w in enumerate(cert["windows"]):
        nm = f"w{i}"
        lines.append(f"; window {i}: [{w['t1']}, {w['t2']}] lhs={w['lhs']:.6f} rhs={w['rhs']:.6f}")
        if INCLUDE_PDC_ASSERTS:
            # If you REALLY want them as constraints, assert <= (not negated)
    ...
    # mutual exclusion for each A/B pair (no overlap in corridor)
    before_flags = []  # for readability in output
    for a in A_corr:
        for b in B_corr:
...
# ---------- write plan.json ----------
def write_plan_json(run_dir: Path, jobs, vals, corridor):
    """Write plan.json: release_times for corridor legs + who-first order list."""
    corr_names = {j["name"] for j in jobs if j["uses_corridor"]}
    release_times = {}
    ...
    for a in A_corr:
        for b in B_corr:
            key_ab = f"{a}_before_{b}"
            key_ba = f"{b}_before_{a}"
            vab = vals.get(key_ab, None)
            vba = vals.get(key_ba, None)
            if vab is True:
    ...
    plan = {
        "t0": time.monotonic(),     # 可换成 gate 构造时的 t0
        "corridor": corridor,
...
def main():
    ...
    if first.strip() == "sat":
        ...
        # print enter times
        e_items = sorted([(k, v) for k, v in vals.items() if k.startswith("e_")], key=lambda kv: kv[0])
        if e_items:
            print("  [enter times]")
            for k, v in e_items:
                try:
        ...
        # print who-first booleans
        bool_items = sorted([(k, v) for k, v in vals.items() if isinstance(v, bool)], key=lambda kv: kv[0])
        if bool_items:
            print("  [who-first flags]")
            for k, v in bool_items:
                print(f"   - {k} : {v}")

        # write plan.json for runtime nodes
        write_plan_json(run_dir, jobs, vals, CORRIDOR)
